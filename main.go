...
// CreateOrUpdate insert the value on duplicate update it, just for mysql.
func (s *DB) CreateOrUpdate(value interface{}, updateValue interface{}) *DB {
	return s.clone().NewScope(value).
		InstanceSet("gorm:update_interface", updateValue).
		callCallbacks(s.parent.callbacks.creates).db
}
...
func (s *DB) slog(sql string, retryIdx int, t time.Time, db interface{}, vars ...interface{}) {
	if s.logMode == detailedLogMode {
		if l, ok := s.logger.(*hlog.Logger); ok && logPrintFunc != nil {
			err := s.Error
			if err == ErrRecordNotFound {
				err = nil
			}
